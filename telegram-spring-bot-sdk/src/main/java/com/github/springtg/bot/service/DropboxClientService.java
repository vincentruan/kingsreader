...
import com.github.springtg.bot.repository.UserTokenRepository;
import com.github.storage.dropbox.config.DropboxConfigProperties;
...
public class DropboxClientService {
    ...
    private static final Logger LOG = getLogger(DropboxClientService.class);
    ...
    public DropboxClientService(final DropboxConfigProperties dropboxConfigProperties, final DbxWebAuth auth, final DbxRequestConfig requestConfig,
    ...
    public String startAuth(final HttpSession session) {
        final DbxSessionStore csrfTokenStore = new DbxStandardSessionStore(session, dropboxConfigProperties.getSessionStore().getKey());
        final DbxWebAuth.Request authRequest = DbxWebAuth.newRequestBuilder()
                .withRedirectUri(dropboxConfigProperties.getRedirectUri(), csrfTokenStore)
                .build();
    ...
    public void finishAuthAndSaveUserDetails(final HttpSession session, final Map<String, String[]> parameterMap) throws Exception {
        final DbxSessionStore csrfTokenStore = new DbxStandardSessionStore(session, dropboxConfigProperties.getSessionStore().getKey());
        final DbxAuthFinish authFinish = auth.finishFromRedirect(dropboxConfigProperties.getRedirectUri(), csrfTokenStore, parameterMap);
